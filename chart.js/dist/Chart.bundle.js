...
},{"6":6}],51:[function(require,module,exports){
'use strict';
...
module.exports = function(Chart) {
	...
	var WindDirectionScale = Chart.LinearScaleBase.extend({
		setDimensions: function() {
			var me = this;
			var opts = me.options;
			var tickOpts = opts.ticks;
		...
		getDistanceFromCenterForValue: function(value) {
			...
			var scalingFactor = me.drawingArea / (me.max - me.min);
			/* if (me.options.reverse) {
				return (me.max - value.x) * scalingFactor;
			}*/
		...
		getBasePosition: function() {
			...
			return me.getPointPositionForValue(0, {x: 0, y: 0});// me.beginAtZero? 0 : min < 0 && max < 0? max : min > 0 && max > 0? min : 0);
		...
		draw: function() {
			...
			if (opts.display) {
				...
				helpers.each(me.ticks, function(label, index) {
					// Don't draw a centre value (if it is minimum)
					if (index > 0 || opts.reverse) {
						var yCenterOffset = me.getDistanceFromCenterForValue(me.ticksAsNumbers[index]); // PERPETRATOR
						var yHeight = me.yCenter - yCenterOffset;
	...
	Chart.scaleService.registerScaleType('windDirection', WindDirectionScale, defaultConfig);
